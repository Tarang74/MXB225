...
\begin{table}[H]
    ...
    \begin{tabular}{ccc}
        ...
        \midrule
        \multicolumn{3}{c}{Real repeated eigenvalues \(\lambda\) with eigenvector \(\symbf{v}\).\ \(\symbf{w}\) satisfies \(\left( \lambda I - \symbf{A} \right) \symbf{w} = \symbf{v}\) (\(\tau^2 - 4\Delta = 0\))}                                                 \\
        \midrule
        \(\lambda < 0\)                                         & Degenerate stable node   & \multirow{2}{*}{\(\symbf{x}\left( t \right) = c_1 e^{\lambda t} \symbf{v} + c_2 \left( t e^{\lambda t} \symbf{v} + e^{\lambda t} \symbf{w} \right)\)}                   \\
        \(\lambda > 0\)                                         & Degenerate unstable node &                                                                                                                                                                         \\
        \midrule
